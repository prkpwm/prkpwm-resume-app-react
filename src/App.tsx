      ...
      <div className="job-top">
        ...
        <div className="job-right">
          <span className="job-period">
            <Ico>{Icons.calendar}</Ico>{job.period}
          </span>
          <span className="job-type-badge">{job.type}</span>
        </div>
      </div>
      <div className="job-body">
        <div className="job-resp-label">KEY RESPONSIBILITIES</div>
        <ul className="job-bullets">
          {job.bullets.map((b) => <li key={b}>{b}</li>)}
        </ul>
        <div className="job-tech-row">
          <span className="job-tech-label">TECH STACK:</span>
          {job.tech.join(', ')}
        </div>
      </div>
    </div>
      ...
      <div className="resume">
        ...
        <aside className="sidebar">
          ...
          <div className="sb-photo">
            <img src={mePhoto} alt="Pakpoom Srisen" className="sb-photo-img" />
          </div>

          {/* Summary */}
          <section className="sb-section">
            <SidebarSectionTitle icon={Icons.user} label="SUMMARY" />
            <p className="sb-summary">{d.summary}</p>
          </section>
          ...
          <section className="sb-section">
            ...
            {d.skills.map((sg) => (
              <div key={sg.category} className="sb-skill-group">
                <div className="sb-skill-cat">{sg.category}</div>
                <div className="sb-skill-badges">
                  {sg.items.map((item) => (
                    <span key={item} className="sb-badge">{item}</span>
                  ))}
                </div>
              </div>
            ))}
          ...
          <section className="sb-section">
            ...
            <ul className="sb-strengths">
              {strengthList.map((s) => (
                <li key={s}>
                  <Ico>{Icons.check}</Ico>{s}
                </li>
              ))}
            </ul>
          </section>
          ...
          <section className="sb-section">
            ...
            <div className="sb-edu-degree">{d.education.degree}</div>
            <div className="sb-edu-school">{d.education.school}</div>
            <div className="sb-edu-period">
              <Ico>{Icons.calendar}</Ico>{d.education.period}
            </div>
          </section>
        </aside>
        ...
        <main className="main">
          ...
          <header className="main-header">
            <div className="header-name-block">
              <div className="header-name">{fn} {ln}</div>
              <div className="header-title">{d.title.replace('\n', ' ')}</div>
            </div>
            <div className="header-contacts">
              <div className="header-contact-row">
                <span className="hc-item"><Ico>{Icons.mail}</Ico>{d.contact.email}</span>
                <span className="hc-item"><Ico>{Icons.phone}</Ico>{d.contact.phone}</span>
                <span className="hc-item"><Ico>{Icons.globe}</Ico>{d.contact.website.replace('https://', '')}</span>
              </div>
              <div className="header-contact-row">
                <span className="hc-item"><Ico>{Icons.linkedin}</Ico>{d.contact.linkedin}</span>
                <span className="hc-item"><Ico>{Icons.pin}</Ico>{d.contact.location}</span>
              </div>
            </div>
          </header>
              ...
              {d.achievements.map((a) => (
                <div key={a.title} className="ach-card">
                  <div className="ach-icon">
                    {achievementIcons[a.title] ?? Icons.trophy}
                  </div>
